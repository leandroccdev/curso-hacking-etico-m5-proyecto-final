...
#<
    def __init__(self, url: str) -> None:
        ...
        #<
        self.url: str = url
        self._test_results: list[XSSTestResult] = []
        #>

    def add_test_result(self, r: XSSTestResult) -> None:
        '''Adds new test result.'''
        self._test_results.append(r)

    def is_vulnerable(self) -> bool:
        '''Check if this target is vulnerable to xss.'''
        #<
...
async def send(s: ClientSession, r_ctx: XSSTestResult, verb: str, url: str,
...
#<
    ...
    async with ClientSession(timeout=ClientTimeout(args.timeout)) as s:
        ...
        calls: list[asyncio.Task[XSSTestResult]]= []
        for verb, url, fields in targets:
            ...
            for t in tests:
                ...
                # Create Task
                #<
                rctx: XSSTestResult = XSSTestResult(t["expect"])
...
#<
    ...
    ap = ArgumentParser(
            add_help = True,
            description="Modulo 5 - Portafolio: " \
            "Automatización de Pruebas de XSS Injection sobre URLs"
        )
    ap.add_argument(
            "-t",
            "--tests",
            action="store",
            dest="tests",
            help="Casos de uso para testear XSS.",
            required=True
        )
    ap.add_argument(
            "-u",
